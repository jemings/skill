...
## hud statusline 스크립트는 별도 표식을 함께 올린다

`statusline-command.sh` / `statusline-tokens.sh` 는 `install.sh` 의 다운그레이드
가드가 비교하는 `# hud-statusline-version: N` 표식을 각각 갖는다. 이는 plugin.json
...
강제하므로, hud 를 수정했다면 커밋 전에 반드시 돌린다:

```bash
bash plugins/hud/skills/hud/scripts/test-install.sh
```
