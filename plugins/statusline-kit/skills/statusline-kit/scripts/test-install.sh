...
# CLAUDE_STATUSLINE_KIT_DEST 로 mktemp 디렉터리에 격리해 실행한다.
#
# 실행:
#   bash skills/statusline-kit/scripts/test-install.sh
...
CLAUDE_STATUSLINE_KIT_DEST="$DEST" bash "$INSTALL" >/dev/null
...
STDERR_OUT="$(CLAUDE_STATUSLINE_KIT_DEST="$DEST" bash "$INSTALL" 2>&1 1>/dev/null)"
...
_marker() { sed -n 's/^# statusline-kit-version: \([0-9][0-9]*\)$/\1/p' "$1" | head -1; }
...
sed "s/^# statusline-kit-version: .*/# statusline-kit-version: 999/" \
...
FORCE=1 CLAUDE_STATUSLINE_KIT_DEST="$DEST" bash "$INSTALL" >/dev/null 2>&1
...
sed "s/^# statusline-kit-version: .*/# statusline-kit-version: 99999999999999999999/" \
  "$DEST/statusline-tokens.sh" >"$DEST/statusline-tokens.sh.new" &&
  mv "$DEST/statusline-tokens.sh.new" "$DEST/statusline-tokens.sh"
STDERR_OUT="$(CLAUDE_STATUSLINE_KIT_DEST="$DEST" bash "$INSTALL" 2>&1 1>/dev/null)"
...
FORCE=1 CLAUDE_STATUSLINE_KIT_DEST="$DEST" bash "$INSTALL" >/dev/null 2>&1  # 원상복구
  ...
  for f in statusline-command.sh statusline-tokens.sh; do
    if head_body="$(git -C "$SCRIPT_DIR" show "HEAD:${PREFIX}${f}" 2>/dev/null)"; then
      if ! printf '%s\n' "$head_body" | diff -q - "$SCRIPT_DIR/$f" >/dev/null 2>&1; then
        head_v="$(printf '%s\n' "$head_body" | sed -n 's/^# statusline-kit-version: 0*\([0-9][0-9]*\)$/\1/p' | head -1)"
