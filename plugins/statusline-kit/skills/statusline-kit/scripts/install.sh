#!/usr/bin/env bash
# statusline-kit 스킬 설치: 두 statusline 스크립트 배치 + settings.json statusLine 병합 + 동작 확인.
# 기존 스크립트가 있고 내용이 다르면 diff 를 경고로 stderr에 출력한 뒤 덮어쓴다.
# 단, 소스의 버전 표식(statusline-kit-version)이 설치본보다 낮으면 다운그레이드로
# 판단해 exit 3 으로 차단한다 — 낡은 플러그인 캐시에서 실행된 경우가 전형.
# FORCE=1 로만 강제 덮어쓸 수 있다.
set -euo pipefail

SRC="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEST="${CLAUDE_STATUSLINE_KIT_DEST:-$HOME/.claude}"
...
_ver() {
  local v
  v=$(sed -n 's/^# statusline-kit-version: 0*\([0-9][0-9]*\)$/\1/p' "$1" 2>/dev/null | head -1) || true
  printf '%s' "${v:-0}"
}
  ...
  if _num_lt "$src_v" "$dest_v" && [ "${FORCE:-0}" != 1 ]; then
    {
      ...
      echo "  claude plugin marketplace update skill && claude plugin update statusline-kit@skill"
...
for f in statusline-command.sh statusline-tokens.sh; do
  ...
  if [ "$(_ver "$SRC/$f")" = "$(_ver "$DEST/$f")" ]; then
    echo "경고: 버전 표식이 같은데 내용이 다릅니다 — 스크립트 수정 시 두 파일의 statusline-kit-version 을 함께 올려야 다운그레이드 가드가 동작합니다." >&2
  fi
